...
class DockerExecObserver:
    ...
    def begin_tool(self, request: ToolBeforeRequest) -> None:
        if not self.enabled or request.resource_scope is not None or request.tool_name == "exec":
            return
        with self._lock:
    ...
    def infer_scope(self, event: ToolCompletedEvent) -> ResourceScope | None:
        if not self.enabled or event.resource_scope is not None or event.execution_id is not None or event.tool_name == "exec":
            return event.resource_scope
    ...
    def _handle_event_line(self, line: str) -> None:
        ...
        exec_id = (
            ...
            or _as_str(attrs.get("execID"))
            or _as_str(attrs.get("execId"))
        )
        if not exec_id:
            return
        container_id = _as_str(attrs.get("container")) or _as_str(attrs.get("containerID"))
